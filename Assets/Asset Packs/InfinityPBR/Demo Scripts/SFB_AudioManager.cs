...
public class SFB_AudioManager : MonoBehaviour
{
    ...
    {
        if (pitchBySpeed)
        {                                                                                       // If we are adjusting pitchBySpeed
    ...
    void UpdateVolume()
    {
        ...
        if (volumeBySpeed)
        {                                                                                   // If volume is being set by speed
    ...
    void PlayAudio(string name)
    {
        ...
        if (Random.Range(0, 100) >= (100 - audioClips[index].chanceOfPlaying))
        {                               // Only if a random chance is positive
            ...
            if (audioClips[index].volumeBySpeed)
            {                                                               // If we are adjusting volume by speed
                                                                            // Clamp the volume
    ...
    {
        if (!loopPlaying)
        {                                       // If a loop isn't already playing
